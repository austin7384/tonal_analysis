...
# criterion. Consumes the three staging CSVs written by build_phrase_matrix.py
# in data/processed/sg_phrase_analysis/, fits Taddy's Distributed Multinomial
# Regression (Poisson-approximation gamma-lasso, BIC-tuned) with a single
# binary group covariate (top vs. bottom Readability quintile), and writes a
# ranked phrase list to outputs/tables/csv/sg_phrase_ranking_readability.csv.
#
# zeta_j > 0  -> phrase appears more in top-Readability abstracts (high_read)
# zeta_j < 0  -> phrase appears more in bottom-Readability abstracts (low_read)
...
IN_DIR   <- file.path(ROOT, "data/processed/sg_phrase_analysis")
OUT_DIR  <- file.path(ROOT, "outputs/tables/csv")
OUT_FILE <- file.path(OUT_DIR, "sg_phrase_ranking_readability.csv")
